...
const getModCallsLogs = async (serverKey, options = {}) => {

    isValidServerKey(serverKey);
    isValidOptions(options);
    if (!isCooldown('getModCallsLogs')) {
        console.log('[ERLC API Wrapper: getModCallsLogs()] Wait for cooldown seconds to request again');
        return;
    }
    ...
    try {
        const response = await apiClient.get(`/server/modcalls`, {
            headers: headers,
        ...
        throw new Error(`Failed to retrieve mod call logs: ${error.message}`);
    }
};

module.exports = getModCallsLogs;
