...
const getKillLogs = async (serverKey, options = {}) => {

    isValidServerKey(serverKey);
    isValidOptions(options);
    if (!isCooldown('getKillLogs')) {
        console.log('[ERLC API Wrapper: getKillLogs()] Wait for cooldown seconds to request again');
        return;
    }
    ...
    try {
        const response = await apiClient.get(`/server/killlogs`, {
            headers: headers,
        ...
        throw new Error(`Failed to retrieve kill logs: ${error.message}`);
    }
};

module.exports = getKillLogs;
