...
const getServerBans = async (serverKey, options = {}) => {

    isValidServerKey(serverKey);
    isValidOptions(options);
    if (!isCooldown('getServerBans')) {
        console.log('[ERLC API Wrapper: getServerBans()] Wait for cooldown seconds to request again');
        return;
    }
    ...
    try {
        const response = await apiClient.get(`/server/bans`, {
            headers: headers,
        ...
        throw new Error(`Failed to retrieve server bans: ${error.message}`);
    }
};

module.exports = getServerBans;
